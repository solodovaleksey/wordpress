...
if ( ! class_exists( 'Polo_TinyMCE_Shortcode' ) ) {

	class Polo_TinyMCE_Shortcode {

		public function __construct() {
			add_action( 'admin_init', array( $this, 'polo_shortcode_button' ) );
			add_action( 'admin_footer', array( $this, 'polo_get_shortcodes' ) );
		}


		public function polo_shortcode_button() {
			if ( current_user_can( 'edit_posts' ) && current_user_can( 'edit_pages' ) ) {
				add_filter( 'mce_external_plugins', array( $this, 'polo_add_buttons' ) );
				add_filter( 'mce_buttons', array( $this, 'polo_register_buttons' ) );
			}
		}

		public function polo_add_buttons( $plugin_array ) {
			$plugin_array['crumshortcodes'] = POLO_ROOT_URL . '/library/inc/shortcodes/js/shortcode-tinymce-button.js';

			return $plugin_array;
		}

		public function polo_register_buttons( $buttons ) {
			array_push( $buttons, 'separator', 'crumshortcodes' );

			return $buttons;
		}

		public function polo_get_shortcodes() {
...
if ( class_exists( 'Polo_TinyMCE_Shortcode' ) ) {
	...
	$Polo_TinyMCE_Shortcode = new Polo_TinyMCE_Shortcode;
	...
	class MCE_Table_Buttons {
		...
		public static function mce_external_plugins( $plugin_array ) {
			...
			if ( version_compare( $tinymce_version, '400', '>' ) ) {
				...
				$plugin_array['table'] = POLO_ROOT_URL . '/library/inc/shortcodes/js/plugin' . $variant . '.js';
