...
class Breadcrumb_Trail {
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			$breadcrumb .= $this->args['wrap_begin'];
			...
			foreach ( $this->items as $item ) {
				...
				$item = !empty( $matches ) ? sprintf( '%s<span itemprop="name">%s</span>%s', $matches[1], $matches[2], $matches[3] ) : sprintf( '<a><span itemprop="name">%s</span></a>', $item );
